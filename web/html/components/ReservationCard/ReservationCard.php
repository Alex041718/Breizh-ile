        ...
        $render =  /*html*/
            ...
            <div class="reservation-card' . ' " id=" ' . $id . '" onclick="redirectToReservationDetails(' . $data->getId() . ')">
                ...
                <div class="reservation-card__content">
                    <div class="reservation-card__content__data-container">
                        <h4 class="reservation-card__content__data-container__title" title="' . $housing->getTitle() . '">' . $housing->getTitle() . '</h4>
                        <div class="reservation-card__content__data-container__description">
                            ' . $housing->getShortDesc() . '
                        </div>
                        <div class="reservation-card__content__data-container__down">
                            <div class="reservation-card__content__data-container__down__ville">
                                <i class="fa-solid fa-location-dot"></i>
                                <div>' . $housing->getAddress()->getPostalCode(). ' - '. $housing->getAddress()->getCity() .'</div>
                            </div>
                            <div class="reservation-card__content__data-container__down__personne">
                                <i class="fa-solid fa-user"></i>
                                <div>' . $data->getNbPerson() . '</div>
                            </div>
                        </div>
                    </div>
                    <div class="reservation-card__content__right">
                        <div class="reservation-card__content__right__prix"> ' . $prixTTC  . '€ TTC</div>
                        <div class="reservation-card__content__right__date">' . $reservation->getBeginDate()->format('d/m/Y') . '</div>
                    </div>
                </div>
            </div>
        ';
